...
{
  options = {
    jellyfin-mpv-shim.enable = lib.mkEnableOption "jellyfin-mpv-shim configuration";
  };

  config = lib.mkIf config.jellyfin-mpv-shim.enable {
    # Remap mpv's default w/e (panscan down/up) to Alt+w / Alt+e
    # so that w remains free for jellyfin-mpv-shim's "mark watched" binding.
    home.file.".config/jellyfin-mpv-shim/input.conf".text = ''
      Alt+w add panscan -0.1
      Alt+e add panscan 0.1
      e ignore
    '';

    # Decouple video decoding from rendering so unfocused windows
    ...
    home.file.".config/jellyfin-mpv-shim/conf.json".text = builtins.toJSON {
      ...
      auto_play = true;
      check_updates = false;
      client_uuid = "fdb26af3-a397-4683-ac03-324e8345db67";
